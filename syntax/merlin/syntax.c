...
/* The Merlin syntax module parses Merlin/Merlin 32 assembler syntax
   ...
   - Flexible string delimiters, ' = high bit, " = normal
*/
...
static char *define_varlabel(const char *name, int len)
{
  ...
  /* Generate new unique name for this definition */
  snprintf(unique_name, sizeof(unique_name), "unid_%d", varlabel_counter++);

  /* Update or set the unique name */
  if (vl->unique_name)
    myfree(vl->unique_name);
...
int isidchar(char c)
{
  /* Merlin: periods and tildes allowed in identifiers */
...
static void handle_at(char *s)
{
  ...
  for (;;) {
    ...
    if (*s && !isspace((unsigned char)*s)) {
      ...
      db = parse_string(&s,delim,8);
      if (db && db->size > 0) {
        /* Invert high bit on last character (XOR 0x80) */
...
static void handle_fls(char *s)
{
  ...
  for (;;) {
    ...
    if (*s && !isspace((unsigned char)*s)) {
      ...
      db = parse_string(&s,delim,8);
      if (db) {
        /* Alternate high bit: even chars normal, odd chars high bit set */
        for (i = 0; i < db->size; i++) {
          if (i & 1)
            db->data[i] |= 0x80;
        }
...
static void handle_op(char *s)
{
  ...
  /* Parse CPU name (alphanumeric identifier) */
  ...
  if (len == 0 || len >= sizeof(cpuname)) {
    syntax_error(24);  /* invalid CPU type for XC directive */
    return;
  }

  /* Copy CPU name to buffer */
  memcpy(cpuname, start, len);
  cpuname[len] = '\0';

  /* Call CPU module's set_cpu_type() */
...
struct {
  ...
  "dsect",handle_dsect,
  "dend",handle_dend,
  "dummy",handle_dsect,  /* Merlin DUM - dummy section start (alias) */
...
static char *parse_label_field(char **start,int *asntype)
{
  ...
  /* Merlin: Check for local label :LABEL (alphanumeric identifier) */
  /* Only treat as label if not spaced (i.e., at start of line) */
  if (!spaced && *s == ':' && ISIDSTART(*(s+1))) {
    /* Merlin local label: :LOOP, :DONE, etc. */
    if (asntype) {
      char *namestart = s + 1;
      ...
      if (merlin_last_global_label == NULL) {
        syntax_error(12);  /* local label without global label context */
        name = NULL;
      }
      else {
        /* Create local label scoped to last global label */
        buf = make_local_label(0, merlin_last_global_label,
                               strlen(merlin_last_global_label),
  ...
  */
    ...
    *start = s;
    ...
    if (!spaced && *s == ']' && ISIDSTART(*(s+1))) {
      ...
      /* Define variable label and get its unique name */
      name = define_varlabel(namestart, nameend - namestart);
      is_special_label = 1;  /* Mark as special label */
    ...
    if (name) {
      ...
      /* Merlin: Check for label suffix ':' (but not :+, :-, or :LABEL) */
      if (s[0]==':' && s[1]!='+' && s[1]!='-' && !ISIDSTART(s[1])) {
        s = skip(s+1);
        spaced = 0;
      }
  ...
  while (line = read_next_line()) {
    ...
    if (labname = parse_label_field(&s,&asn_type)) {
      ...
      if (asn_type) {
        ...
        /* A global label doesn't start with a space, :, ], or . */
        /* (local labels created by make_local_label start with space) */
        if (labname[0] != ' ' && labname[0] != ':' && labname[0] != ']' && labname[0] != '.') {
...
/* Merlin: Handle three-tier labels: global, :LABEL local, ]LABEL variable */
{
  ...
  /* Merlin: Check for local label reference :LABEL */
  if (*s == ':' && ISIDSTART(*(s+1))) {
    char *namestart = s + 1;
    char *nameend = namestart;
    ...
    /* Create local label scoped to last global label */
    name = make_local_label(n, merlin_last_global_label,
                           strlen(merlin_last_global_label),
                           namestart, nameend - namestart);
    *start = skip(nameend);
  }
  ...
  /* Merlin: Check for variable label reference ]LABEL */
    ...
    if (unique != NULL) {
      ...
      syntax_error(42);  /* variable label referenced before definition */
      *start = nameend;
      return NULL;
    }
    *start = skip(nameend);
