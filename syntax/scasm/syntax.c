...
int isidchar(char c)
{
  /* SCASM: periods NOT allowed in identifiers (reserved for directives/local labels) */
...
struct {
  ...
  "inb",handle_incbin,   /* SCASM: .INB - include binary */
  "defc",handle_defc,
  "xdef",handle_global,
  "xref",handle_global,
  "lib",handle_global,
...
static char *parse_label_field(char **start,int *asntype)
{
  ...
  /* Only treat as label if not spaced (i.e., at start of line) */
    ...
    if (asntype) {
      ...
      if (labnum < 1 || labnum > 255) {
        syntax_error(11);  /* invalid local label number (must be 1-255) */
        name = NULL;
      }
...
void parse(void)
{
...
#endif
    /* SCASM: * in column 1 is always a comment */
    if (parse_end || *line=='*')
      continue;
  ...
  /* SCASM: Output A2osX build metadata if extracted from source */
  if (scmasm_target_file || scmasm_source_file) {
    fprintf(stderr, "\n");
    if (scmasm_target_file)
      fprintf(stderr, "A2osX Target Binary: %s\n", scmasm_target_file);
    if (scmasm_source_file)
      fprintf(stderr, "A2osX Source Listing: %s\n", scmasm_source_file);
  }
}
...
/* SCASM: Handle three-tier labels: global, :N private, .N local */
{
  ...
  /* SCASM: Check for local label reference .N */
    ...
    labnum = atoi(numstart);
    if (labnum < 1 || labnum > 255) {
      syntax_error(11);  /* invalid local label number */
      *start = numend;
      return NULL;
    }
