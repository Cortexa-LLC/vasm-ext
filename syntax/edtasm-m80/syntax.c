...
{
  /* Z80 EDTASM-M80: only semicolon is a comment character */
  if (*s == commentchar)
    return 1;
...
int isidchar(char c)
{
  /* Z80 EDTASM-M80: identifiers can contain alphanumeric, underscore, @
     Note: Dollar sign ($) not supported (6809 OS-9 specific) */
...
/* M80 Segment Directives */
static void handle_aseg(char *s)
/* ASEG - Absolute segment (M80)
   Code/data assembled at absolute addresses set by ORG */
{
  section *sec;
  sec = new_section("ASEG",code_type,1);
  set_section(sec);
  sec->flags |= ABSOLUTE;
}

static void handle_cseg(char *s)
/* CSEG - Code segment (M80)
   Default code segment (in Phase 1 MVP: absolute output) */
{
  section *sec;
  sec = new_section("CSEG",code_type,1);
  set_section(sec);
  sec->flags |= ABSOLUTE;  /* MVP: absolute output (Phase 2: relocatable) */
}

static void handle_dseg(char *s)
/* DSEG - Data segment (M80)
   Separate data area for variables and constants */
{
  section *sec;
  sec = new_section("DSEG",data_type,1);
  set_section(sec);
  sec->flags |= ABSOLUTE;
}
...
static void handle_end(char *s)
{
   ...
   *
   * Note: The execution address is primarily informational in EDTASM-M80.
   * Use the -exec=<symbol> command line option to specify the start symbol
   * for output formats that support it.
   */
  parse_end = 1;
...
/* M80 string comparison conditionals - aliases for IFC/IFNC */
static void handle_ifidn(char *s)
{
  ifc(s,1);  /* IFIDN = IF identical (same as IFC) */
}

static void handle_ifdif(char *s)
{
  ifc(s,0);  /* IFDIF = IF different (same as IFNC) */
}
...
struct {
  ...
  /* EDTASM data directives (6809 names) */
  "fcb",0,handle_d8,
  "fdb",0,handle_d16,
  "fcc",0,handle_fcc,
  "fcs",0,handle_fcs,
  ...
  /* Z80 EDTASM directives (TRS-80 Model III names) */
  "defb",0,handle_d8,           /* Define Byte (same as FCB) */
  "defw",0,handle_d16,          /* Define Word - 16-bit little-endian (same as FDB) */
  ...
  "dw",0,handle_d16,            /* Define Word (M80 alias) */
  "ds",0,handle_rmb,            /* Define Storage (M80 alias) */
  "dc",0,handle_d8,             /* Define Constant (M80 - typically 8-bit) */

  /* M80 segment directives */
  "aseg",0,handle_aseg,         /* Absolute segment */
  "cseg",0,handle_cseg,         /* Code segment */
  "dseg",0,handle_dseg,         /* Data segment */
  ...
  "ifndef",DT_IF,handle_ifnd,     /* Assemble if symbol not defined */
  "ifb",DT_IF,handle_ifb,         /* Assemble if argument blank */
  "ifnb",DT_IF,handle_ifnb,       /* Assemble if argument not blank */
  "ifidn",DT_IF,handle_ifidn,     /* Assemble if arguments identical */
  "ifdif",DT_IF,handle_ifdif,     /* Assemble if arguments different */
...
int init_syntax(void)
{
  ...
  /* Z80 EDTASM-M80: Default to case-insensitive mode per M80/EDTASM spec
     (Unlike 6809 EDTASM which is case-sensitive by default) */
  nocase = 1;
  nocase_macros = 1;
  ...
  cond_init();
  current_pc_char = '*';  /* Asterisk is current PC symbol in Z80 EDTASM-M80 */
...
{
  if (!strcmp(p,"-nocase")) {
    /* Z80 EDTASM-M80: Already case-insensitive by default
       This flag is accepted for compatibility but has no effect */
    nocase = 1;
    nocase_macros = 1;
    return 1;
  }
  /* Z80 note: -auto-branch removed (6809-specific feature) */
  return 0;
}
