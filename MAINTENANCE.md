...
When a new vasm version is released (e.g., 2.1), use the automated maintenance scripts:

```bash
# 1. Update vasm repository (upstream tracking)
cd /Users/bryanw/Projects/Vintage/tools/vasm
./update_upstream.sh
# Choose 'release' or 'daily', script handles download, extraction, commit, and tagging

# 2. Integrate into vasm-ext
cd /Users/bryanw/Projects/Vintage/tools/vasm-ext
./integrate_upstream.sh
# Script guides through: fetch, merge, conflict resolution, testing, and tagging
```

**What the scripts automate:**

**update_upstream.sh** (in vasm repository):
- Downloads latest release or daily build
- Extracts and overwrites repository files
- Commits with detailed message
- Tags the version (e.g., v2.1)
- Prompts to push to GitHub

**integrate_upstream.sh** (in vasm-ext repository):
- Fetches from upstream-repo
- Updates upstream-tracking branch
- Creates integration branch
- Merges upstream changes
- Guides through conflict resolution if needed
- Prompts for testing both modules
- Merges to main with --no-ff
- Tags extension release (e.g., v2.1-ext1)
- Prompts to push to GitHub

**Manual steps if conflicts occur:**

The integration script will pause if conflicts are detected. Resolve them following these guidelines:
...
#   - Makefile (keep our modifications if any)
#   - Any core files we patched (check CLAUDE.md for documented fixes)

# After resolving:
git add <resolved-files>
git commit
# Then continue with script instructions
```
